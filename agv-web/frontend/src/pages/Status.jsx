...
export default function Status({ usuario }) {
  ...
  const agvResponse = await fetch(`${API_BASE_URL}/dispositivos/${agvSelecionado}`);
        const agvData = await agvResponse.json();
        setStatusAgv(agvData);

        // Carregar pedidos ativos para este AGV específico
  const pedidosResponse = await fetch(`${API_BASE_URL}/pedidos?dispositivo_id=${agvSelecionado}&status=pendente,em_andamento,coletando`);
        ...
        if (pedidoAtivo && pedidoAtivo.status === 'pendente') {
          try {
            ...
            await fetch(`${API_BASE_URL}/pedidos/${pedidoAtivo.id}/iniciar`, {
              method: 'PUT'
            });
  ...
  const cancelarPedido = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/pedidos/${pedidoAtual.id}/cancelar`, {
        method: 'PUT',
  ...
  const removerItemDaRota = async (itemIndex) => {
    ...
    try {
      ...
      if (!pedidoAtual.id.toString().startsWith('demo_')) {
        const response = await fetch(`${API_BASE_URL}/pedidos/${pedidoAtual.id}/remover-item`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const cancelarRotaCompleta = async () => {
    ...
    try {
      ...
      if (!pedidoAtual.id.toString().startsWith('demo_')) {
        const response = await fetch(`${API_BASE_URL}/pedidos/${pedidoAtual.id}/cancelar-completo`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
